...
//global vars
var instruments,contract_quantity,btc_quantity,scaled_side_bid,scaled_side_ask,scaled_side_error;
var quantity,quantity_error,order_count,order_count_error,range_start,range_start_error,range_end,range_end_error;
var hidden,noise,noise_amount,pnoise,pnoise_amount,shape_flat,shape_asc,shape_desc,shape_custom;
var scaled_preview_btn,submit_new_scaled;
var rows,scaled_preview_wrap;
var lblQuantity,lblPriceLower,lblPriceUpper;
var instrumentsData,minOrderSize=.01;
var currentUrl;
...
var current_price_btn,save_preset_btn,preset_name,preset_list,delete_presets;
var presets=[];
var presetUID=1;
...
function init() {
	...
	chrome.storage.local.get({rootUrl:"https://www.deribit.com"},function(items) {
...
//	console.log("presets1:"+JSON.stringify(presets));
	presets=presets.filter((preset)=>{return preset.id!=this.id});
...
function checkCurrentUrl() {
	...
	chrome.tabs.query({active: true,windowId:bgnd.parentWindowId},function(tabs) {
		currentUrl=tabs[0].url;
		//if current page url is not deribit, open a new deribit tab
		console.log("currentUrl:"+currentUrl+" index: "+currentUrl.indexOf("deribit.com"));
		if(currentUrl.indexOf(rootUrl)!=0) {
			chrome.tabs.create({windowId:bgnd.parentWindowId,url:rootUrl},(tab)=>{
...
//	}	
	...
	var m=0,v=[],amounts=[],dPrice=(settings.rangeEnd-settings.rangeStart)/(settings.orderCount-1),prices=[];
	//get prices
	for(var p=0;p<settings.orderCount;p++) {
		...
		if(p==0) {
			...
			if(settings.pnoise) {
				price=addNoise(price,settings.pnoiseAmount,settings.rangeStart,settings.rangeEnd,0);
			}
		}
	...
	switch(settings.shape) {
		case "flat":
			amounts=shapeAmounts(settings.quantity,settings.orderCount,[20, 20, 20, 20, 20]);
			break;
		case "asc":
			amounts=shapeAmounts(settings.quantity, settings.orderCount,[1, 12.5, 25, 37.5, 50]);
			break;
		case "desc":
			amounts=shapeAmounts(settings.quantity, settings.orderCount, [50, 37.5, 25, 12.5, 1]);
			break;
			//		case "custom":
	...
	if(settings.noise) {
		amounts=amounts.map(function(e) {return addNoise(e,settings.noiseAmount,minOrderSize,999999,0)});
	}

	//adjust amounts to total quantity to be traded
	var k=amounts.reduce(function(a, b) {return a+b;},0);
    ...
    k=amounts.reduce(function(a, b) {return a+b;},0);
    ...
    if(settings.type=="Contract") {
        amounts=amounts.map(function(e) {
        	return (e<1.0?Math.ceil(e):Math.floor(e));
        });   	
    }
    ...
    var totals=prices.map(function(v,i) {
    ...
    if(e.target.preview) {
		...
		for(var header=true,j=settings.orderCount-1;j>=0;j--,header=false) {
		...
		chrome.tabs.query({active:true,windowId:bgnd.parentWindowId},function(tabs) {
			...
			chrome.tabs.sendMessage(tabs[0].id,{orders:orders,rootUrl:rootUrl,side:settings.side.toLowerCase()},function(response) {
				if(response) {
...
function addNoise(value,noiseAmount,range_start,range_end,level) {
	var randomPosNeg=Math.random()-.5<0?-1:1;
	var percentageChange=(100+Math.random()*noiseAmount*randomPosNeg)/100;
...
function shapeAmounts(totalAmount,orderCount,shapeArray) {
	function interpolate(t) {
		var e,i,n=0;
		if(Object.keys(o).map(function(t) {return Number(t)}).sort(function(t, e) {return t>e?1:-1}).map(function(r) {t==r && (i=o[r]),e || (t<r?e=r:n=r)}),i) return i;
	...
	var shapeArrayTotal=shapeArray.reduce(function(a, b) {return a+b;},0);
	console.log("shapeArrayTotal: "+shapeArrayTotal);

	//get the delta change of the order count
	var dOrderCount=(shapeArray.map(function(t) {return t/shapeArrayTotal;}),1/(orderCount-1));
	console.log("dOrderCount: "+dOrderCount);

	//create shape interpolation map
	var o={};
	for(var i=0;i<shapeArray.length;i++) { // if shapeArray.length is 10, then 0 to 9
	...
	//total all amounts
	var amountsTotal=s.reduce(function(a, b) {return a+b;},0);
	var ratio=amountsTotal/totalAmount;
	
	//scale all amounts by ratio
	return s=s.map(function(t) {return t/ratio;});
}
...
function validateAll() {
	...
	if(parseFloat(range_end.value)<=parseFloat(range_start.value)) {
		range_end_error.textContent="The price upper must be greater than the price lower.";
...
//	,shape_flat,shape_asc,shape_desc,shape_custom
	...
	if(btc_quantity.checked) {
		...
		if(settings.quantity<settings.orderCount) {
			quantity_error.textContent="The contract quantity must be as much as than the order count.";
			quantity_error.className="error";
			order_count_error.textContent="The order count must be less or equal to the contract quantity.";
			order_count_error.className="error";
			success=false;
		}
		//check to be sure order count<=quantity
		if(settings.quantity!=Math.floor(settings.quantity)) {
			quantity_error.textContent="The contract quantity must an integer amount.";
			quantity_error.className="error";
			success=false;
		}
	}
...
function validateOrderCount() {
	...
	if(value.length==0) {
		...
		order_count_error.textContent="Please enter a value greater than or equal to 2.";
		order_count_error.className="error";
		return false;
	} else if(n>50) {
		order_count_error.textContent="Please enter a value less than or equal to 50.";
		order_count_error.className="error";
		return false;
	} else {
	...
	getInstruments(function(result) {
		if(result!==null) {
			if(result.success) {
				...
				//only show futures
				instrumentsData=instrumentsData.filter((obj)=>{return obj.kind==="future"});
